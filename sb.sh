...
red "The script does not support your current system, please choose to use Ubuntu, Debian, Centos system." && exit
fi
...
blue "Sing-box kernel version successfully installed: $(/etc/s-box/sing-box version | awk '/version/{print $NF}')"
else
red "The download of Sing-box kernel is incomplete and the installation failed. Please run the installation again" && exit
fi
else
red "Failed to download Sing-box kernel, please run the installation again and check whether the VPS network can access Github" && exit
fi
}
inscertificate(){
ymzs(){
ym_vl_re=www.yahoo.com
blue "Vless-reality's SNI domain name defaults to www.yahoo.com"
...
yellow "1: No! Use self-signed certificate (default by pressing Enter)"
...
green "If there is a domain name that has been resolved, do you want to apply for an Acme domain name certificate? (It forms a dual certificate mode, which can coexist with a self-signed certificate, and each protocol can be switched independently)"
yellow "1: No! Use self-signed certificate (default by pressing Enter)"
...
do
[[ -n $(ss -tunlp | grep -w udp | awk '{print $5}' | sed 's/.*://g' | grep -w "$port") || -n $(ss -tunlp | grep -w tcp | awk '{print $5}' | sed 's/.*://g' | grep -w "$port") ]] && yellow "\nThe port is occupied, please re-enter Port" && readp "Custom port:" port
done
else
until [[ -z $(ss -tunlp | grep -w udp | awk '{print $5}' | sed 's/.*://g' | grep -w "$port") && -z $(ss -tunlp | grep -w tcp | awk '{print $5}' | sed 's/.*://g' | grep -w "$port") ]]
do
...
vlport(){
readp "\nSet Vless-reality port [1-65535] (Enter to skip to a random port between 2000-65535):" port
chooseport
port_vl_re=$port
}
vmport(){
readp "\nSet Vmess-ws port [1-65535] (Enter to skip to a random port between 2000-65535):" port
chooseport
port_vm_ws=$port
}
hy2port(){
readp "\nSet Hysteria2 main port [1-65535] (Enter to skip to a random port between 2000-65535):" port
chooseport
port_hy2=$port
}
tu5port(){
readp "\nSet Tuic5 main port [1-65535] (Enter to skip to a random port between 2000-65535):" port
chooseport
port_tu=$port
}
import(){
...
echo
blue "Confirmation of each protocol port is as follows"
...
changeym(){
[ -f /root/ygkkkca/ca.log ] && ymzs="$yellow switches to domain name certificate: $(cat /root/ygkkkca/ca.log 2>/dev/null)$plain" || ymzs="$ yellow has not applied for a domain name certificate and cannot switch $plain"
vl_na="Domain name certificate in use: $(jq -r '.inbounds[0].tls.server_name' /etc/s-box/sb.json). $yellow replaces the domain name certificate that meets the requirements of reality. It is not recommended to use Own resolved domain name $plain"
tls=$(jq -r '.inbounds[1].tls.enabled' /etc/s-box/sb.json)
[[ "$tls" = "false" ]] && vm_na="TLS is currently turned off. $ymzs ${yellow} switches to turn on TLS, the Argo tunnel will be closed, you can enter the main menu option 4 and change the port to https 443 The port of the system, the main protocol can realize CDN preferred IP ${plain}" || vm_na="The domain name certificate in use: $(cat /root/ygkkkca/ca.log 2>/dev/null). $yellow switches to Turn off TLS, Argo tunnel will be available, you can enter the main menu option 4, change the port to http 80 series port, the main protocol can achieve CDN preferred IP$plain"
hy2_sniname=$(jq -r '.inbounds[2].tls.key_path' /etc/s-box/sb.json)
...
red "Only option 1 (vless-reality) is supported. Because the domain name certificate has not been applied for, the certificate switching options for vmess-ws, Hysteria-2, and Tuic-v5 are not displayed yet"
fi
green "0: Return to the upper level"
readp "Please select:" menu
if [ "$menu" = "1" ]; then
readp "Please enter the vless-reality domain name (enter and use www.yahoo.com):" menu
...
red "No domain name certificate is currently applied for and cannot be switched. Select 12 in the main menu and execute Acme certificate application" && sleep 2 && sb
fi
elif [ "$menu" = "3" ]; then
if [ -f /root/ygkkkca/ca.log ]; then
...
allports
green "vless-reality and vmess-ws can only change unique ports"
...
green "0: Return to the upper level"
readp "Please select the protocol [0-4] to change the port:" menu
if [ "$menu" = "1" ]; then
vlport
...
yellow "Hysteria2 is not configured with multiple ports" && changeport
fi
else
changeport
fi
elif [ "$menu" = "4" ]; then
green "1: Replace the Tuic5 main port (the original multi-port is automatically reset and deleted)"
...
echo
readp "Enter custom uuid, it must be in uuid format. If you don't understand, press Enter (reset and randomly generate uuid):" menu
if [ -z "$menu" ]; then
uuid=$(/etc/s-box/sing-box generate uuid)
else
uuid=$menu
...
readp "Enter Telegram robot user ID: " userid
telegram_id=$userid
echo '#!/bin/bash
export LANG=en_US.UTF-8
...
res=$(timeout 20s curl -s -X POST $URL -d chat_id=telegram_id -d parse_mode=${MODE} --data-urlencode "text=🚀[ Vmess-ws sharing link]: supports v2rayng, nekobox, small rocketshadowrocket "$'"'"'\n\n'"'"'"${message_text_m2}")
fi
if [[ -n $(ps -ef | grep cloudflared) && -s '/etc/s-box/argo.log' ]]; then
...
res=$(timeout 20s curl -s -X POST $URL -d chat_id=telegram_id -d parse_mode=${MODE} --data-urlencode "text=🚀[Hysteria-2 Sharing Link]: Support nekobox, shadowrocket "$'"'"'\n\n'"'"'"${message_text_m5}")
...
res=$(timeout 20s curl -s -X POST $URL -d chat_id=telegram_id -d parse_mode=${MODE} --data-urlencode "text=🚀[Sing-box configuration file]: supports SFA, SFI, SFW "$'"'"'\n\n'"'"'"${message_text_m7}")
res=$(timeout 20s curl -s -X POST $URL -d chat_id=telegram_id -d parse_mode=${MODE} --data-urlencode "text=🚀[Clash-meta configuration file]: Support CMFA, CMFW-V , CMFOC "$'"'"'\n\n'"'"'"${message_text_m8}")
if [ $? == 124 ]; then
echo TG_api request timed out, please check whether the network has been restarted and whether you can access TG
fi
...
sbymfl(){
[[ $(systemctl is-active warp-svc) = active ]] && warp_s4_ip="Current IP: $(curl -4sx socks5h://localhost:40000 icanhazip.com -k)" || warp_s4_ip='No warp- IPV4 for s5, blacklist mode'
[[ $(systemctl is-active warp-svc) = active ]] && warp_s6_ip="Current IP: $(curl -6sx socks5h://localhost:40000 icanhazip.com -k)" || warp_s6_ip='No warp- IPV6 for s5, blacklist mode'
v4v6
if [[ -z $v4 ]]; then
...
green "VPS local outbound distribution. If warp option 1 is installed, the local IP will be taken over by the warp (options 5 and 6)"
yellow "Supports full domain name mode (eg: www.google.com) and geosite mode (eg: netflix, disney, openai)"
yellow "Note:"
yellow "1: Only the complete domain name can be filled in with the full domain name method, and only geosite can be filled in with the geosite method"
yellow "2: Do not divert the same complete domain name or geosite repeatedly"
...
sbymfl
echo
green "1: Reset warp-wireguard-ipv4 distribution domain name $wfl4"
green "2: Reset warp-wireguard-ipv6 offload domain name $wfl6"
green "3: Reset warp-socks5-ipv4 offload domain name $sfl4"
green "4: Reset warp-socks5-ipv6 offload domain name $sfl6"
green "5: Reset VPS local ipv4 offload domain name $adfl4"
...
if [ "$menu" = "1" ]; then
readp "Leave a space between each domain name, and press Enter to skip to reset and clear the full domain name diversion channel of warp-wireguard-ipv4):" w4flym
if [ -z "$w4flym" ]; then
w4flym='"yg_kkk"'
else
...
readp "1: Use the full domain name method\n2: Use the geosite method\n3: Return to the upper level\nPlease select:" menu
if [ "$menu" = "1" ]; then
readp "Leave a space between each domain name and press Enter to skip to reset and clear the full domain name diversion channel of warp-wireguard-ipv6:" w6flym
if [ -z "$w6flym" ]; then
w6flym='"yg_kkk"'
else
...
readp "1: Use the full domain name method\n2: Use the geosite method\n3: Return to the upper level\nPlease select:" menu
if [ "$menu" = "1" ]; then
readp "Leave a space between each domain name and press Enter to skip, which means resetting and clearing the full domain name diversion channel of warp-socks5-ipv4:" s4flym
if [ -z "$s4flym" ]; then
s4flym='"yg_kkk"'
else
...
readp "1: Use the full domain name method\n2: Use the geosite method\n3: Return to the upper level\nPlease select:" menu
if [ "$menu" = "1" ]; then
readp "Leave a space between each domain name and press Enter to skip, indicating that the full domain name of warp-socks5-ipv6 is reset and cleared:" s6flym
if [ -z "$s6flym" ]; then
s6flym='"yg_kkk"'
else
...
readp "1: Use the full domain name method\n2: Use the geosite method\n3: Return to the upper level\nPlease select:" menu
if [ "$menu" = "1" ]; then
readp "Leave a space between each domain name and press Enter to skip the diversion channel that resets and clears the full domain name of the VPS local IPv4:" ad4flym
if [ -z "$ad4flym" ]; then
ad4flym='"yg_kkk"'
else
...
readp "Leave a space between each domain name and press Enter to skip to reset and clear the geosite distribution channel of the VPS local IPv4:" ad4flym
if [ -z "$ad4flym" ]; then
ad4flym='"yg_kkk"'
else
ad4flym="$(echo "$ad4flym" | sed 's/ /","/g')"
...
readp "1: Use the full domain name method\n2: Use the geosite method\n3: Return to the upper level\nPlease select:" menu
if [ "$menu" = "1" ]; then
readp "Leave a space between each domain name and press Enter to skip, which means resetting and clearing the full domain name of the VPS local IPv6 diversion channel:" ad6flym
if [ -z "$ad6flym" ]; then
ad6flym='"yg_kkk"'
else
...
readp "Leave a space between each domain name and press Enter to skip to reset and clear the geosite distribution channel of the VPS local IPv6:" ad6flym
if [ -z "$ad6flym" ]; then
ad6flym='"yg_kkk"'
else
ad6flym="$(echo "$ad6flym" | sed 's/ /","/g')"
...
red "Sing-box was not installed properly" && exit
fi
readp "1: Restart\n2: Shut down\nPlease select:" menu
if [ "$menu" = "1" ]; then
...
red "Sing-box-yg was not installed properly" && exit
fi
lnsb
curl -sL https://gitlab.com/rwkgyg/sing-box-yg/-/raw/main/version/version | awk -F "Update content" '{print $1}' | head -n 1 > /etc /s-box/v
...
sbactive(){
if [[ ! -f /etc/s-box/sb.json ]]; then
red "Sing-box did not start normally, please uninstall and reinstall or select 10 to view the running log feedback" && exit
fi
}
sbshare(){
...
red "File directory/etc/s-box/clash_meta_client.yaml, copy and build according to yaml file format" && sleep 2
echo
...
echo
white "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ~~~~~~"
red "🚀[ vless-reality, vmess-ws, Hysteria2, Tuic5 ] SFA/SFI/SFW configuration files are displayed as follows:"
...
red "File directory/etc/s-box/sing_box_client.json, copy and build according to json file format" && sleep 2
echo
...
sbshare > /dev/null 2>&1
white "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ~~~~~~"
red "🚀[Hysteria-2] v2rayn configuration file is shown as follows:"
red "Please download the official Hysteria2 client core, which supports multi-port hopping and multi-port multiplexing"
red "File directory/etc/s-box/v2rayn_hy2.yaml, copy and build according to yaml file format" && sleep 2
echo
...
white "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ~~~~~~"
red "🚀[ Tuic-v5 ] v2rayn configuration file is shown as follows:"
red "Please download the Tuic5 official client core. If you have set up multiple ports, please modify it yourself. Multi-port jumping is not supported and multi-port multiplexing is supported."
red "File directory/etc/s-box/v2rayn_tu5.json, copy and build according to json file format" && sleep 2
echo
...
if [[ $ww4 = *"WEI"* && $ww6 = *"WEI"* && $ws4 = *"WEI"* && $ws6 = *"WEI"* && $l4 = *"WEI"* && $l6 = *"WEI"* ]] ; then
...
echo -e "${bblue} ░█${plain}█ ░██ ██ ██ ░██ ░░${red}██ ░██ ░░██ ░██ ░░██ ${plain } "
white "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
white "Yong Ge Github project: github.com/yonggekkk"
...
echo -e "The latest Sing-box-yg script version number detected: ${yellow}${latestV}${plain} (can select 7 for update)"
...
echo
echo -e "The latest official version of the current Sing-box kernel: ${bblue}${inscore}${plain} (installed)"
echo
echo -e "The current Sing-box latest beta version kernel: ${bblue}${precore}${plain} (switchable)"
else
echo
echo -e "The current Sing-box has installed the official version of the kernel: ${bblue}${inscore}${plain}"
...
echo
echo -e "The latest official version of the current Sing-box kernel: ${bblue}${latcore}${plain} (switchable)"
else
echo
echo -e "The current Sing-box has a beta version of the kernel installed: ${bblue}${inscore}${plain}"
...
echo -e "The current Sing-box latest beta kernel: ${bblue}${precore}${plain}"
fi
red "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
...
echo -e "Sing-box status: $yellow has not started. You can choose 6 to restart. If it is still the same, choose 10 to view the log and give feedback. It is recommended to uninstall and reinstall Sing-box$plain"
else
